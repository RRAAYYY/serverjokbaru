...
export async function POST(req: NextRequest) {
  try {
    ...
    await runQuery(`UPDATE keys SET device_fingerprint = NULL, locked_to_device = 0, status = 'active' WHERE key_code = ?`, [keyCode.toUpperCase()]);
    
    await runQuery(`
      INSERT INTO logs (action, admin_id, admin_username, details, ip, timestamp)
      VALUES (?, ?, ?, ?, ?, ?)
    `, ['key_reset', decoded.id, decoded.username, `Reset HWID for key: ${key.key_code} (${key.name})`, req.headers.get('x-forwarded-for') || 'unknown', new Date().toISOString()]);
